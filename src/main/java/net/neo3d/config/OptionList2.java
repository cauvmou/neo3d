...
public class OptionList2 extends ContainerObjectSelectionList<OptionList2.Entry> {
    ...
    public void addButton(OptionWidget widget) {
        this.addEntry(new net.neo3d.config.OptionList2.Entry(widget));
    }

    public void addAll(Option<?>[] options) {
        for (int i = 0; i < options.length; i++) {
            this.addEntry(new net.neo3d.config.OptionList2.Entry(options[i].createOptionWidget((int) (0.1f * width), 0, (int) (0.8f * width), 20)));
//            this.addEntry(new Entry(options[i].createOptionWidget(width / 2 - 155, 0, 200, 20)));
    ...
    @Nullable
    protected net.neo3d.config.OptionList2.Entry getEntryAtPos(double x, double y) {
        ...
        if (x < this.getScrollbarPosition() && x >= (double)k && x <= (double)l && n >= 0 && m >= 0 && n < this.getItemCount()) {
            return (net.neo3d.config.OptionList2.Entry)this.children().get(n);
        }
        return null;
    }
...
//        return this.width / 2 + 124 + 32;
    ...
    public Optional<OptionWidget> getHoveredButton(double mouseX, double mouseY) {
        for (net.neo3d.config.OptionList2.Entry buttonEntry : this.children()) {
    ...
    protected void renderList(GuiGraphics guiGraphics, int x, int y, int mouseX, int mouseY, float delta) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            net.neo3d.config.OptionList2.Entry entry = this.getEntry(j);
    ...
    protected static class Entry
            extends ContainerObjectSelectionList.Entry<net.neo3d.config.OptionList2.Entry> {
